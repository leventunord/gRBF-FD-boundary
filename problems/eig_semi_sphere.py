...
def eig_semi_sphere(N=6400, l=4, K=25, l_grad=3, K_grad=20, seed=None, num_eigs=30):
    #-- PARAMETERS --#
    ...
    x_sym = sp.sin(phi) * sp.cos(theta)
    y_sym = sp.sin(phi) * sp.sin(theta)
    z_sym = sp.cos(phi)
    ...
    # sample boundary
    x_sym_bd = sp.sin(phi_max) * sp.cos(theta)
    y_sym_bd = sp.sin(phi_max) * sp.sin(theta)
    z_sym_bd = 0

    bd = Manifold([theta], [x_sym_bd, y_sym_bd, z_sym_bd])
    bd.sample([theta_range], num_boundary)

    manifold.params = np.vstack([
        manifold.params,
        np.insert(bd.params, 1, values=phi_max, axis=1),
    ])
