...
def generate_sphere_interface(N, R=0.5):
    ...
    num_boundary = int(2 * np.sqrt(N * np.pi))
    num_interior = (N - num_boundary) // 2

    theta_boundary = np.random.uniform(0, 2*np.pi, size=num_boundary)
    phi_boundary = np.full(num_boundary, np.pi/2)
    params_bd = np.column_stack([theta_boundary, phi_boundary])
    
    x_bd = R * np.sin(phi_boundary) * np.cos(theta_boundary)
    y_bd = R * np.sin(phi_boundary) * np.sin(theta_boundary)
    z_bd = R * np.cos(phi_boundary)
    ...
    for mf, params_int, points_int in zip(
        ...
        mf.id_boundary = np.arange(num_interior, num_interior + num_boundary)
        mf.build_tree()

    n_vecs_bd = np.zeros((num_boundary, 3))
    n_vecs_bd[:, 2] = 1.0 

    return manifold_p, manifold_m, n_vecs_bd
...
def compute_mms_interface(manifold_p, manifold_m, n_vecs_bd, beta_p, beta_m, kappa=1.0):
    R_val = 0.5
    ...
    flux_p = np.sum(n_vecs_bd * u_grad_p[id_bd_p], axis=1)
    flux_m = np.sum(n_vecs_bd * u_grad_m[id_bd_m], axis=1)
    psi_vals = beta_p * flux_p - beta_m * flux_m

    return u_p, u_m, f_p, f_m, q_vals, psi_vals

def build_operators(manifold, n_vecs_bd, K, l, K_grad, l_grad, is_plus=True):
...
def interface_sphere(N=6400, l=4, K=25, l_grad=4, K_grad=35, seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    manifold_p, manifold_m, n_vecs_bd = generate_sphere_interface(N)

    u_p, u_m, f_p, f_m, q_vals, psi_vals = compute_mms_interface(
        manifold_p, manifold_m, n_vecs_bd, beta_p, beta_m, kappa
    )

    L_p, D_n_p = build_operators(manifold_p, n_vecs_bd, K, l, K_grad, l_grad, is_plus=True)
    L_m, D_n_m = build_operators(manifold_m, n_vecs_bd, K, l, K_grad, l_grad, is_plus=False)
...
if __name__ == "__main__":
    ie = interface_sphere(N=3200, seed=1)
    print(f"ie: {ie:.3e}")