...
def poisson_robin_semi_torus_mod(N=6400, l=4, K=25, l_grad=3, K_grad=20, seed=None, better_stencil=False, better_qp=False):
    #-- PARAMETERS --#
    ...
    # # delete points near boundary
    # area_semi_torus = 2 * np.pi**2 * R * r
    # h_est = np.sqrt(area_semi_torus / N)
    # margin_ratio = 0.6 
    # phi_margin = (margin_ratio * h_est) / R 

    # print(phi_margin)
    
    # phi_range_interior = (phi_min + phi_margin, phi_max - phi_margin)

    # manifold.sample([theta_range, phi_range_interior], num_interior)
